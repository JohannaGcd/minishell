...
/*   Updated: 2025/05/18 20:49:31 by sveta         ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
...
pid_t	fork_child(t_minishell *mshell, t_command *curr_cmd, int *pipe_fd, int prev_read_end, int *exit_status)
{
	pid_t	child_id;
	...
	if (child_id == 0)
	{
		execute_child(mshell, curr_cmd, pipe_fd, prev_read_end, exit_status);
	}
	return (child_id);
}

void	execute_pipeline(t_minishell *mshell, int *exit_status,
		int *nbr_children, int *prev_read_end)
{
	t_command	*curr_cmd;
	int			pipe_fd[2];
	pid_t		child_id;
	...
	{
		(*nbr_children)++;
		if (setup_pipe(pipe_fd) == -1 || fork_child(mshell, curr_cmd,
				pipe_fd, *prev_read_end, exit_status) == -1)
		{
	...
	child_id = execute_last_cmd(mshell, curr_cmd, *prev_read_end, exit_status);
...
int	execute_multiple_cmd(t_minishell *mshell, int *exit_status)
{
	...
	execute_pipeline(mshell, exit_status, &nbr_children, &prev_read_end);
	return (0);
}
