...
// 127 if command not found, 126 if permission denied
...
int	execute_child(t_minishell *mshell, t_command *curr_cmd, char **envp, int *pipe_fd, int prev_read_end, int *exit_status)
{
	char	*command_wp;
...
int	execute_last_cmd(t_minishell *mshell, t_command *curr_cmd, char **envp, int prev_read_end, int *exit_status)
{
	...
	if (child_id == 0)
	{
		...
		if (!is_builtin_cmd(curr_cmd->command_args))
		{
			command_wp = return_command_with_path(curr_cmd->command_args[0],
					mshell);
			if (execve(command_wp, curr_cmd->command_args, envp) == -1)
			{
				perror("execve failed");
